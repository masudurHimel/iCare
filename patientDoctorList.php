        ...
        <div id="mid">
            <form action="" method="post">
                <div class="cardInner">
                    ...
                    <label><b>Which kind of specialist you are looking for ?</b></label> <br>
                    <input type="text" name="specialistType" id="specialistType" placeholder="Heart/Cardiologist/Gynecologist ect">
                    <input type="submit" name="docSearch" id="docSearch" value="Submit">
                    <?php
                    // Check connection
                    $link = mysqli_connect("localhost", "root", "", "icare");
                    if (isset($_POST['docSearch'])) {
                        $specialistType = mysqli_real_escape_string($link, $_REQUEST['specialistType']);
                        if ($link === false) {
                            die("ERROR: Could not connect. " . mysqli_connect_error());
                        }
// Attempt select query execution
                        $sql = "SELECT Name,Gender,BMDC_RegNo,MBBS_Session,PostGrad_deatails FROM doctor where PostGrad_deatails LIKE '%$specialistType%'";
                        ...
                        if ($result = mysqli_query($link, $sql)) {
                            if (mysqli_num_rows($result) > 0) {
                                ...
                                while ($row = mysqli_fetch_array($result)) {
                                    ...
                                    echo $row['PostGrad_deatails'] . "<br><br><br><br>";
